layui.define(['table', 'jquery', 'form'], function (exports) {
    ...
    */
    tableSelect.prototype.render = function (opt) {
        ...
        opt.defaultVal = opt.defaultVal || [];
        opt.showClear = opt.showClear || false;
        opt.reset = opt.reset || false;
        ...
        elem.off('click').on('click', function(e) {
            ...
            var tableBox = '<div class="tableSelect layui-anim layui-anim-upbit" style="left:'+l+';top:'+t+';border: 1px solid #d2d2d2;background-color: #fff;box-shadow: 0 2px 4px rgba(0,0,0,.12);padding:10px 10px 0 10px;position: absolute;z-index:66666666;margin: 5px 0;border-radius: 2px;min-width:530px;">';
                ...
                tableBox += '</form>';
                tableBox += '<button style="float:right;" class="layui-btn layui-btn-sm tableSelect_btn_select"><i class="layui-icon layui-icon-ok" style="font-size: 12px!important;"></i>选择<span></span></button>';
                if (opt.showClear) {
                    tableBox += '<button style="float:right; margin-right: 10px;" class="layui-btn layui-btn-sm layui-btn-primary tableSelect_btn_clear"><i class="layui-icon layui-icon-close" style="font-size: 12px!important;"></i>清除<span></span></button>';
                }
                tableBox += '</div>';
            ...
            function setChecked (res, curr, count) {
				...
				if(checkStatus.isAll){
					$('#'+tableName+'').next().find('.layui-table-header th[data-field="' + opt.checkedKey + '"] input[type="checkbox"]').prop('checked', true);
					$('#'+tableName+'').next().find('.layui-table-header th[data-field="' + opt.checkedKey + '"] input[type="checkbox"]').next().addClass('layui-form-checked');
				}
				updataButton(checkedData.length)
            }
            ...
            function defaultChecked (res, curr, count){
                if(opt.checkedKey && (elem.attr('ts-selected') || opt.defaultVal.length)){
                    var selected = elem.attr('ts-selected').split(",");
                    if (opt.reset || !selected.length) {
